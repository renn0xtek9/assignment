...
# In this sample application, the cross toolchain is configured to use a
# compiler for the RTEMS operating system targeting the "pc686" BSP

# Note that to use this, the "RTEMS" platform module may need to be added to the
# system-wide CMake installation as a default CMake does not yet recognize RTEMS
# as a system name.  An example of this is distributed with the pc-rtems PSP.
...
# RTEMS_DYNAMIC_LOAD definition: - Set to FALSE for platforms that create a
# RTEMS executable and link it to the cFE core. - Set to TRUE for platforms that
# expect the cFE core to to be dynamically loaded into an existing runtime
# image. This is tied to the OSAL-BSP and PSP implementation so generally cannot
...
# 0x10000 (64K) AND <= 0x97C00 (607K) OR >= 0x100000 (1024K). The memory | top
# is of course another limit. Make sure there is enough space before the | upper
# memory limits for the image and the memory allocated by it to fit. | Make sure
# the value you choose is aligned to 4 bytes.
# +--------------------------------------------------------------------------+
SET (RTEMS_RELOCADDR 0x00100000)

# Exception handling is very iffy.  These two options disable eh_frame creation.
SET (CMAKE_C_COMPILE_OPTIONS_PIC -fno-exceptions
                                 -fno-asynchronous-unwind-tables)

# Link libraries needed for a RTEMS 5+ executable This was handled by the
# bsp_specs file in 4.11
SET (LINK_LIBRARIES "-lrtemsdefaultconfig -lrtemsbsp -lrtemscpu")

# +---------------------------------------------------------------------------+
# | Common RTEMS toolchain statements
# +---------------------------------------------------------------------------+
# The TOOLS and BSP are allowed to be installed in different locations. If the
# README was followed they will both be installed under $HOME By default it is
# assumed the BSP is installed to the same directory as the tools
SET (
  RTEMS_TOOLS_PREFIX
  "$ENV{HOME}/rtems-${CMAKE_SYSTEM_VERSION}"
  CACHE PATH "RTEMS tools install directory")
SET (
  RTEMS_BSP_PREFIX
  "${RTEMS_TOOLS_PREFIX}"
  CACHE PATH "RTEMS BSP install directory")
